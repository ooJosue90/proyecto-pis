...
final class AdminAgricultureRepository
{
        ...
        try {
            $cultivos = $this->rows(
                'SELECT c.*, u.nombre AS agricultor_nombre,
                        COALESCE((
                            SELECT GROUP_CONCAT(ca.nombre ORDER BY ca.id_cultivo_asociado SEPARATOR \', \')
                            FROM cultivos_asociados ca WHERE ca.id_cultivo = c.id_cultivo
                        ), \'\') AS cultivos_asociados,
            ...
            $lotes = $this->rows(
                'SELECT l.*, c.tipo AS cultivo_tipo, c.nombre AS cultivo_nombre, u.nombre AS agricultor_nombre
                 FROM lotes l
                 LEFT JOIN cultivos c ON l.id_cultivo = c.id_cultivo
                 LEFT JOIN usuarios u ON c.id_usuario = u.id_usuario
                 ORDER BY l.etapa_actual ASC, l.id_lote DESC'
            );
            $statsCultivos = $this->one(
                'SELECT COUNT(*) AS total_cultivos,
                        COUNT(DISTINCT tipo) AS tipos_diferentes,
                        (SELECT COUNT(DISTINCT codigo) FROM cultivos_asociados) AS asociados_diferentes
                 FROM cultivos'
            );
    ...
    public function cropDetail(int $id): ?array
    {
        return $this->preparedOne(
            'SELECT c.*, u.nombre AS agricultor,
                    COALESCE((
                        SELECT GROUP_CONCAT(ca.nombre ORDER BY ca.id_cultivo_asociado SEPARATOR \', \')
                        FROM cultivos_asociados ca WHERE ca.id_cultivo = c.id_cultivo
                    ), \'\') AS cultivos_asociados,
    ...
    {
        return $this->preparedOne(
            'SELECT l.*, CONCAT(c.nombre, \' · \', c.tipo) AS cultivo, c.fecha_siembra, u.nombre AS agricultor
             FROM lotes l
             LEFT JOIN cultivos c ON l.id_cultivo = c.id_cultivo
             LEFT JOIN usuarios u ON c.id_usuario = u.id_usuario
             WHERE l.id_lote = ?',
